...
Since the scratch workspace is shared, only one running instance of micro is allowed to persist it at a time, to avoid two instances overwriting each other's `scratch.json`. The first scratch instance to start claims a lock file, `ConfigDir/workspaces/scratch.lock`, and becomes the instance responsible for both restoring the previous session at startup and saving it back. Every later scratch instance, while that lock is held, still runs a fully functional scratch workspace, but it starts blank (it does not restore the persisted session) and never saves to `scratch.json`, and it shows a one-line notice explaining why. This keeps the owning instance's session entirely its own: an ephemeral instance can neither show nor clobber it.
