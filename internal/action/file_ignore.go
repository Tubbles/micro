...
// ignoreMatcher matches paths against a stack of gitignore-syntax
// rule sets. The type is named for the syntax it parses, not for
// the source filename: the same parser handles .gitignore, .ignore,
// .fdignore, .npmignore, .eslintignore, etc., differing only in
// what the loader picks up. v1 only loads .gitignore; future loaders
// can plug in to the same matcher.
type ignoreMatcher struct {
	levels []ignoreLevel
}

// newIgnoreMatcher walks root depth-first, loads each .gitignore it
// finds, and skips descending into directories that the rules built
// so far have already excluded. Skipping ignored subtrees both
// avoids loading their .gitignore files (irrelevant: their contents
// are already ignored) and keeps the build cheap on repos with
// large vendored trees.
...
func (m *ignoreMatcher) loadDir(absDir, relDir string) {
	if rules, err := loadIgnoreFile(filepath.Join(absDir, ".gitignore")); err == nil && len(rules) > 0 {
	...
	for _, e := range entries {
		...
		if relDir == "" {
			...
			subRel = relDir + "/" + e.Name()
		}
		if m.Match(subRel, true) {
			continue
		}
		m.loadDir(filepath.Join(absDir, e.Name()), subRel)
	}
}
